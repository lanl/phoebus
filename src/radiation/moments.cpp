...
//                  const Real scattering_fraction, typename CLOSURE::LocalGeometryType
//                  &g, Real (&U_mhd_0)[4], Real (&U_rad_0)[4], const int &k, const int
//                  &j, const int &i)
...
//    } W = std::sqrt(1. + W); Vec con_v{P_mhd[1] / W, P_mhd[2] / W, P_mhd[3] / W};
...
//    Real W = 0.;
//    SPACELOOP2(ii, jj) { W += (*gcov_)[ii + 1][jj + 1] * P_mhd[ii + 1] * P_mhd[jj + 1];
//    } W = std::sqrt(1. + W); Real cov_v[3] = {0}; SPACELOOP2(ii, jj) { cov_v[ii] +=
//    (*gcov_)[ii + 1][jj + 1] * P_mhd[ii + 1] / W; } Real vdH = 0.; SPACELOOP(ii) { vdH
//    += P_mhd[ii + 1] / W * P_rad[ii + 1]; }
//
...
//                       const MeanOpacity &mean_opacity, const Real &rho, const Real
//                       &ug0, const Real &Ye, const Real J0[3], const int &nspec, const
//                       RadiationType species[3], const Real &scattering_fraction, const
//                       Real &dtau)
...
//        Real sdetgam = geom.DetGamma(CellLocation::Cent, iblock, k, j, i);
//        typename CLOSURE::LocalGeometryType g(geom, CellLocation::Cent, iblock, k, j,
//        i);
//
//        // Bounds
//        Real xi_max;
...
//          // rootfind.
//          Real P_mhd_guess[4] = {ug, v(iblock, pv(0), k, j, i), v(iblock, pv(1), k, j,
//          i),
//                                 v(iblock, pv(2), k, j, i)};
//          Real U_mhd_0[4];
...
//                                       species_d[ispec], con_tilPi, cov_g,
//                                       con_gamma.data, alpha, beta, sdetgam,
//                                       scattering_fraction, g, U_mhd_0, U_rad_0, k, j,
//                                       i);
//
...
//          auto status = srm.CalculateRadPrimitive(P_mhd_guess, U_rad_guess,
//          P_rad_guess); srm.CalculateSource(P_mhd_guess, P_rad_guess, dS_guess); for
//          (int n = 0; n < 4; n++) {
...
//                P_mhd_mag_min = std::min<Real>(P_mhd_mag_min,
//                std::fabs(P_mhd_guess[m]));
//              }
//            }
//
...
//                constexpr Real Jmin = 1.e-20; // TODO(BRR) needs to be a bit smaller
//                than
...
//                P_rad_guess); srm.CalculateSource(P_mhd_guess, P_rad_guess, dS_guess);
...
//                //                printf("nan resid[%i]! %i %i %i Umhdg: %e %e %e %e
//                dSg:
//                //                %e %e %e %e\n", n,
...
//          //                i), v(iblock, idx_F(ispec, 2), k, j, i), dE[ispec] *
//          sdetgam,
...
//            //                   v(iblock, pv(2), k, j, i), v(iblock, idx_J(ispec), k,
//            j,
//            //                   i), v(iblock, idx_H(ispec, 0), k, j, i), v(iblock,
//            //                   idx_H(ispec, 1), k, j, i), v(iblock, idx_H(ispec, 2),
//            k,
//            //                   j, i));
