...
class ClosureM1 : public ClosureEdd<Vec, Tens2, SET> {
 ...
 protected:
  ...
  /// J and \tilde H_i.
  KOKKOS_FUNCTION
  ClosureStatus M1FluidPressureTensor(const Real J, const Vec cov_H, Tens2 *con_tilPi,
                                      Vec *con_tilf);

  //-------------------------------------------------------------------------------------
  /// Calculate the residuals of the M1 root equations
  KOKKOS_FUNCTION
  ClosureStatus M1Residuals(const Real E, const Vec cov_F, const Real xi, const Real phi,
                            const Vec con_tilg, const Vec con_tild, Real *fXi,
                            Real *fPhi);
  ...
  KOKKOS_FUNCTION
  ClosureStatus M1FluidPressureTensor(const Vec cov_F, const Real xi, const Real phi,
  ...
  if (std::isnan(xi) || iter >= max_iter) {
    if (!(xi < 1.e-4) && !(std::fabs(fXi) < 1.e-3)) {
      printf("Con2Prim (Fail) : E = %e F = (%e, %e, %e) \n"
             "                 xi = %e phi = %e fXi = %e fPhi = %e v = (%e, %e, %e) xig "
             "= %e phig = %e\n",
             E, cov_F(0), cov_F(1), cov_F(2), xi, phi, fXi, fPhi, con_v(0), con_v(1),
             con_v(2), xi_guess, phi_guess);
    }
...
KOKKOS_FUNCTION ClosureStatus ClosureM1<Vec, Tens2, SET>::M1FluidPressureTensor(
  ...
  SPACELOOP2(i, j) {
    (*con_tilPi)(i, j) = (*con_tilf)(i) * (*con_tilf)(j) -
                         (W2 * con_v(i) * con_v(j) + gamma->con_gamma(i, j)) / 3;
    (*con_tilPi)(i, j) *= athin;
  }
