 ...
 public:
  ...
  void MetricDerivative(CellLocation loc, int b, int k, int j, int i,
    ...
    SPACETIMELOOP3(mu, nu, sigma) {
      int offst = (sigma - 1) * Utils::SymSize(NDFULL) + Utils::Flatten2(mu, nu, NDFULL);
      dg[mu][nu][sigma] = (sigma == 0) ? 0 : pack_(b, idx_[loc].dg + offst, k, j, i);
    }
  }
  KOKKOS_INLINE_FUNCTION
...
// Check to see if the coordinate system needs to be set
template <typename System>
bool CachedCoordinatesNeedSetting(MeshBlockData<Real> *rc) {
  auto pmb = rc->GetParentPointer();
  auto system_analytic = GetCoordinateSystem<System>(rc);
  auto system_cached = GetCoordinateSystem<CachedOverMeshBlock<System>>(rc);
  ...
  int fail_cnt;
  parthenon::par_reduce( // do along a ghost line
      parthenon::loop_pattern_mdrange_tag, "Check if alpha is correctly cached",
      DevExecSpace(), kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i, int &f) {
        const Real alpha_a = system_analytic.Lapse(CellLocation::Cent, k, j, i);
        const Real alpha_c = system_cached.Lapse(CellLocation::Cent, k, j, i);
        if (std::abs(alpha_a - alpha_c) > SMALL) {
          f += 1;
        }
      },
