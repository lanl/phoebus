...
//Setup based on Magnetically modified spherical accretion in GRMHD: reconnection-driven convection and jet propagation
//By Ressler, Quataert, White, and Blaes 2001
...
void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  PARTHENON_REQUIRE(typeid(PHOEBUS_GEOMETRY) == typeid(Geometry::FMKS),
                    "Problem \"mmsacc\" requires \"FMKS\" geometry!");
  ...
  PARTHENON_REQUIRE_THROWS(eos_type == "IdealGas",
                           "MMSACC setup only works with ideal gas");
  const Real gam = pin->GetReal("eos", "Gamma");
  const Real Cv = pin->GetReal("eos", "Cv");
  const Real n = 1.0 / (gam - 1.0);
  PARTHENON_REQUIRE_THROWS(std::fabs(n - Cv) < 1.e-12, "MMSACC requires Cv = 1/(Gamma-1)");
  const Real cs0 = pin->GetOrAddReal("mmsacc", "cs0", 0.01);
  const Real psi = pin->GetOrAddReal("mmsacc", "psi", 0.0);
  const Real jitter = pin->GetOrAddReal("mmsacc", "jitter", 0.01);
  const int seed = pin->GetOrAddInteger("mmsacc", "seed", time(NULL));
  ...
  const Real b_rad = pin->GetOrAddReal("mmsacc", "b_radius", 10.);
  const Real r_cut = pin->GetOrAddReal("mmsacc", "r_cut", 10.0);
  ...
  pmb->par_for(
      "Phoebus::ProblemGenerator::mmsacc", kb.s, kb.e, jb.s, jb.e, ib.s, ib.e,
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        if (r > r_cut) {
          ...
          v(itmp, k, j, i) = cs0; //get_bondi_temp(r, n, C1, C2, Tc, rs);
          v(irho, k, j, i) = 1.0; //std::pow(v(itmp, k, j, i), n);
  ...
  pmb->par_for(
      ...
      KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        Real sth = std::sin(th); Real cth = std::cos(th);
        Real sph = std::sin(x3); Real cph = std::cos(x3);

        Real Amp = -r*cth*std::sin(psi) + r*sth*cph*std::cos(psi);
        Amp = Amp*std::exp(1.0 - (b_rad*b_rad)/(r*r));

        A(0, k, j, i) = Amp*sth*sph;
        A(1, k, j, i) = Amp*cth*sph;
        A(2, k, j, i) = Amp*cth;
      });
  ...
  if (ibhi > 0) {
    pmb->par_for(
        "Phoebus::ProblemGenerator::MMSACC3", kb.s, kb.e, jb.s, jb.e - 1, ib.s, ib.e - 1,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
          ...
          Real dx1 = coords.CellWidthFA(X1DIR, k,j,i)*gamdet*4; //include determinant and averaging factor of 1/4?
          Real dx2 = coords.CellWidthFA(X2DIR, k,j,i)*gamdet*4;
          Real dx3 = coords.CellWidthFA(X3DIR, k,j,i)*gamdet*4;
...
void PostInitializationModifier(ParameterInput *pin, Mesh *pmesh) {
    const bool magnetized = pin->GetOrAddBoolean("mmsacc", "magnetized", true);
    const Real beta_target = pin->GetOrAddReal("mmsacc", "target_beta", 100.);
    const bool harm_style_beta =
      pin->GetOrAddBoolean("mmsacc", "harm_beta_normalization", true);

    Real beta_min, beta_pmax;
    ComputeBetas(pmesh, 0.0001, beta_min, beta_pmax);
    if (parthenon::Globals::my_rank == 0) {
      printf("MMSACC before normalization: beta_min, beta_pmax = %.14e %.14e\n", beta_min,
             beta_pmax);
    }
    ...
    if (parthenon::Globals::my_rank == 0) {
      printf("mmsacc normalization factor = %.14e\n", B_field_fac);
    }
    ...
    if (parthenon::Globals::my_rank == 0) {
      printf("MMSACC after normalization: beta_min, beta_pmax = %.14e %.14e\n", beta_min_new,
             beta_pmax_new);
    }
}
