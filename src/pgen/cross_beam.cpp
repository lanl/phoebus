...
void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  PARTHENON_REQUIRE(
      ...
      "Problem \"cross_beam\" requires \"Minkowski\" geometry!");
  ...
  const Real rho0 = pin->GetOrAddReal("cross_beam", "rho0", 1.e-12);
  const Real Tg0 = pin->GetOrAddReal("cross_beam", "Tg0", 1.e-12);
  const Real Tr0 = pin->GetOrAddReal("cross_beam", "Tr0", 1.e-2);
  const Real Ye0 = pin->GetOrAddReal("cross_beam", "Ye0", 0.5);
 ...
 /// AK: I think this fixes the junk
//RadiationType dev_species[3] = {species[0], species[1], species[2]};
  ...
  pmb->par_for(
      "Phoebus::ProblemGenerator::cross_beam", kb.s, kb.e, jb.s, jb.e, ib.s,
      ib.e, KOKKOS_LAMBDA(const int k, const int j, const int i) {
        ...
        Real xmax = coords.Xc<1>(ib.e); // is this right way to get xmax?
