...
namespace homologous {
  ...
  pmb->par_for(
	...
	const Real u = phoebus::energy_from_rho_P(eos, mass_density_dev.interpToReal(r),pressure_dev.interpToReal(r),emin,emax,lambda[0]);
	const Real sie = u/mass_density_dev.interpToReal(r);
        const Real T = eos.TemperatureFromDensityInternalEnergy(mass_density_dev.interpToReal(r), sie, lambda);
