...
def sodsolution(pld,prd,rhold,rhord,timed=0.1,x0=0.5):
    ...
    ms = newton(msfunc,2.,args=args)

    #Now find the rest of the solutions
    gp1 = gamma + 1.
    gm1 = gamma - 1.
    p1 = pr*(2.*gamma/gp1*ms**2 - gm1/gp1)
    rho1 = rhor/(2./gp1/ms**2 + gm1/gp1)
    u1 = 2./gp1*(ms - 1./ms)
    ...
    ue = 2./gp1*(al + (x-x0)/time)
    ae = al - gm1*ue*0.5
    pe = pl*(ae/al)**(2.*gamma/gm1)
    rhoe = gamma*pe/ae**2
    ...
    x = np.append([0.,x1],x)
    x = np.append(x,[x2,x3,x3,x4,x4,1.])
...
def msfunc(ms,pl,pr,al,ar,gam):
    gp1 = gam+1.
    gm1 = gam-1.
    f = ms - 1./ms - al/ar*gp1/gm1 * (1. - (pr/pl*(2.*gam/gp1*ms**2 - gm1/gp1))**(gm1/(2.*gam)) )
    print(f)
    return f
