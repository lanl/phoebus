...
def HomSphereRadius(r, Rsphere, B, kappa):
  ...
  def gOut(mu):
    return np.where(mu>np.sqrt(1-(Rsphere*Rsphere)/(r*r)), np.sqrt(1-(r*r/(Rsphere*Rsphere))*(1-mu*mu)), 0)
  def s(mu):
    return np.where(r<Rsphere, r/Rsphere*mu + gIn(mu), 2*gOut(mu))
  
  (J, err)= integrate.quad(lambda x: 1-np.exp(-kappa*Rsphere*s(x)), -1, 1)
  (H, err) = integrate.quad(lambda x: x*(1-np.exp(-kappa*Rsphere*s(x))), -1, 1)
...
parser.add_argument('-k', '--kappa', dest='kappa', default=10, action="store", type=float, 
                    help='Background opacity inside the sphere')
parser.add_argument('-r', '--radius', dest='radius', default=1, action="store", type=float, 
                    help='Sphere radius')
parser.add_argument('-B', dest='B', default=1.0, action="store", type=float, 
...
plt_ax.text(0.05*(xh-xl)+xl, 0.95*(xh-xl)+xl, '$\kappa={}$'.format(args.kappa))
