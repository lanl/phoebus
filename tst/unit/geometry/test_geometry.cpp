      ...
      THEN("The coordinate system can be called on device") {
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA(const int i, int &update) {
              ...
              if (gamma[0][0] != 1)
                update += 1;
              if (GetDifference(gamma[1][1], r * r) > EPS)
                update += 1;
              if (GetDifference(gamma[2][2], std::pow(r * std::sin(th), 2)) >
                  EPS)
                update += 1;
              if (gamma[0][1] != 0)
                update += 1;
              ...
              if (GetDifference(gamma[1][1], 1 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(gamma[2][2], std::pow(r * std::sin(th), -2)) >
                  EPS)
                update += 1;
              ...
              if (GetDifference(Gamma[1][2][2], -r) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][3][3],
                                -r * std::sin(th) * std::sin(th)) > EPS)
                update += 1;
              if (GetDifference(Gamma[2][1][2], r) > EPS)
                update += 1;
              if (GetDifference(Gamma[2][3][3],
                                -r * r * std::cos(th) * std::sin(th)) > EPS)
                update += 1;
              if (GetDifference(Gamma[3][2][3],
                                r * r * std::cos(th) * std::sin(th)) > EPS)
                update += 1;
              if (GetDifference(Gamma[3][1][3],
                                r * std::sin(th) * std::sin(th)) > EPS)
                update += 1;
            },
            n_wrong);
      ...
      THEN("The coordinate system can be called on device") {
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA(const int i, int &update) {
              ...
              if (GetDifference(alpha, std::sqrt(r / (r - 2))) > EPS)
                update += 1;
              // Shift
              Real beta[NDSPACE];
              ...
              if (beta[1] != 0)
                update += 1;
              if (GetDifference(beta[2], -2 * a / (r - 2)) > EPS)
                update += 1;
              // metric
              Real gamma[NDSPACE][NDSPACE];
              system.Metric(0, r, th, ph, gamma);
              if (GetDifference(gamma[2][2],
                                (r - 2) / (r * (a * a + (r - 2) * r))) > EPS)
                update += 1;
              // Inverse metric
              Real g[NDFULL][NDFULL];
              ...
              if (GetDifference(connection[1][1][1],
                                (r - a * a) / (r * r * r)) > EPS) {
                update += 1;
              }
            },
      ...
      THEN("The coordinate system can be called on device") {
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA(const int i, int &update) {
              ...
              if (GetDifference(gdet, r * r) > EPS)
                update += 1;
              // Shift
              Real beta[NDSPACE];
              system.ContravariantShift(2, r, th, ph, beta);
              if (GetDifference(beta[0], 2 / (2 + r)) > EPS)
                update += 1;
              ...
              if (GetDifference(gamma[1][1], r * r) > EPS)
                update += 1;
              if (GetDifference(gamma[2][2], r * r + ((a * a * (2 + r)) / r)) >
                  EPS)
                update += 1;
              if (GetDifference(gamma[0][2], -a * (r + 2) / r) > EPS)
                update += 1;
              // Inverse metric
              ...
              if (gamma[0][1] != 0 || gamma[1][0] != 0 || gamma[1][2] != 0 ||
                  gamma[2][1] != 0)
                update += 1;
              if (GetDifference(gamma[0][0],
                                ((a * a) / (r * r)) + (r / (2 + r))) > EPS)
                update += 1;
              if (GetDifference(gamma[1][1], (1 / (r * r))) > EPS)
                update += 1;
              if (GetDifference(gamma[2][2], (1 / (r * r))) > EPS)
                update += 1;
              if (GetDifference(gamma[2][0], a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(gamma[0][2], a / (r * r)) > EPS)
                update += 1;
              // Metric Derivative
              Real dg[NDFULL][NDFULL][NDFULL];
              ...
              for (int mu = 0; mu < 2; ++mu) {
                for (int nu = 0; nu < 2; ++nu) {
                  if (GetDifference(dg[mu][nu][1], -2 / (r * r)) > EPS)
                    update += 1;
                }
              }
              if (GetDifference(dg[2][2][1], 2 * r) > EPS)
                update += 1;
              if (GetDifference(dg[3][3][1], 2 * r - (2 * a * a / (r * r))) >
                  EPS)
                update += 1;
              if (GetDifference(dg[0][3][1], 2 * a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(dg[3][0][1], 2 * a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(dg[1][3][1], 2 * a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(dg[3][1][1], 2 * a / (r * r)) > EPS)
                update += 1;
              if (dg[0][2][1] != 0 || dg[2][0][1] != 0 || dg[1][2][1] != 0 ||
                  dg[2][1][1] != 0 || dg[2][3][1] != 0 || dg[3][2][1] != 0)
                update += 1;
              SPACETIMELOOP2(mu, nu) {
                if (std::abs(dg[mu][nu][2]) > EPS)
                  update += 1;
                if (dg[mu][nu][3] != 0)
                  update += 1;
              }
              ...
              if (std::abs(Gamma[0][0][0]) > EPS ||
                  std::abs(Gamma[0][0][2]) > EPS ||
                  std::abs(Gamma[0][0][3]) > EPS ||
                  std::abs(Gamma[0][1][2]) > EPS ||
                  std::abs(Gamma[0][2][0]) > EPS ||
                  std::abs(Gamma[0][2][1]) > EPS ||
                  std::abs(Gamma[0][2][2]) > EPS ||
                  std::abs(Gamma[0][2][3]) > EPS ||
                  std::abs(Gamma[0][3][0]) > EPS ||
                  std::abs(Gamma[0][3][2]) > EPS ||
                  std::abs(Gamma[0][3][3]) > EPS)
                update += 1;
              if (GetDifference(Gamma[0][0][1], -1 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[0][1][0], -1 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[0][1][1], -2 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[0][3][1], a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[0][1][3], a / (r * r)) > EPS)
                update += 1;
              if (std::abs(Gamma[1][0][1]) > EPS ||
                  std::abs(Gamma[1][1][0]) > EPS ||
                  std::abs(Gamma[1][0][2]) > EPS ||
                  std::abs(Gamma[1][2][0]) > EPS ||
                  std::abs(Gamma[1][1][2]) > EPS ||
                  std::abs(Gamma[1][2][1]) > EPS ||
                  std::abs(Gamma[1][1][3]) > EPS ||
                  std::abs(Gamma[1][3][1]) > EPS ||
                  std::abs(Gamma[1][2][3]) > EPS ||
                  std::abs(Gamma[1][3][2]) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][0][0], 1 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][1][1], -1 / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][2][2], -r) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][3][3], ((a * a) / (r * r)) - r) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][0][3], -a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[1][3][0], -a / (r * r)) > EPS)
                update += 1;
              for (int mu = 0; mu < NDFULL; ++mu) {
                for (int nu = 0; nu < NDFULL; ++nu) {
              ...
              if (GetDifference(Gamma[2][2][1], r) > EPS)
                update += 1;
              if (GetDifference(Gamma[2][1][2], r) > EPS)
                update += 1;
              if (std::abs(Gamma[3][0][0]) > EPS ||
                  std::abs(Gamma[3][0][2]) > EPS ||
                  std::abs(Gamma[3][2][0]) > EPS ||
                  std::abs(Gamma[3][0][3]) > EPS ||
                  std::abs(Gamma[3][3][0]) > EPS ||
                  std::abs(Gamma[3][1][2]) > EPS ||
                  std::abs(Gamma[3][2][1]) > EPS ||
                  std::abs(Gamma[3][2][2]) > EPS ||
                  std::abs(Gamma[3][2][3]) > EPS ||
                  std::abs(Gamma[3][3][2]) > EPS ||
                  std::abs(Gamma[3][3][3]) > EPS)
                update += 1;
              if (GetDifference(Gamma[3][0][1], a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[3][1][0], a / (r * r)) > EPS)
                update += 1;
              if (GetDifference(Gamma[3][1][1], 2 * a / (r * r)) > EPS)
                update += 1;
              ...
              if (std::abs(da[0]) > EPS || std::abs(da[2]) > EPS ||
                  std::abs(da[3]) > EPS)
                update += 1;
              if (GetDifference(da[1], 1 / (2 * r + r * r)) > EPS)
                update += 1;
            },
            n_wrong);
        REQUIRE(n_wrong == 0);
...
TEST_CASE("McKinneyGammieRyan", "[geometry]") {
  GIVEN("The parameters for a modifier") {
    ...
    WHEN("We create a modifier object") {
      McKinneyGammieRyan transformation(derefine_poles, hslope, poly_xt,
                                        poly_alpha, x0, mks_smooth);
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA(const int i, int &update) {
              ...
              if (GetDifference(C[2], 3.2724923474893677) > EPS)
                update += 1;
              // Jcov
              if (std::abs(Jcov[0][1] > EPS))
                update += 1;
              if (std::abs(Jcov[0][2] > EPS))
                update += 1;
              if (std::abs(Jcov[1][2] > EPS))
                update += 1;
              if (std::abs(Jcov[2][0] > EPS))
                update += 1;
              if (std::abs(Jcov[2][1] > EPS))
                update += 1;
              if (GetDifference(Jcov[2][2], 1) > EPS)
                update += 1;
              ...
              if (GetDifference(Jcov[1][1], 2.29713534) > EPS)
                update += 1;
              // Jcon
              if (std::abs(Jcon[0][1] > EPS))
                update += 1;
              if (std::abs(Jcon[0][2] > EPS))
                update += 1;
              if (std::abs(Jcon[1][2] > EPS))
                update += 1;
              if (std::abs(Jcon[2][0] > EPS))
                update += 1;
              if (std::abs(Jcon[2][1] > EPS))
                update += 1;
              if (GetDifference(Jcon[2][2], 1) > EPS)
                update += 1;
      ...
      THEN("The coordinate system can be called on device") {
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA(const int i, int &update) {
              ...
              if (GetDifference(alpha, 0.7811769941965497) > EPS)
                update += 1;
              // metric determinant
              Real gdet = system.DetG(X[0], X[1], X[2], X[3]);
              if (GetDifference(gdet, 64.40965903079623) > EPS)
                update += 1;
              // Spacetime metric
              Real g[NDFULL][NDFULL];
              system.SpacetimeMetric(X[0], X[1], X[2], X[3], g);
              if (GetDifference(g[0][0], -0.3612937485396168) > EPS)
                update += 1;
              if (GetDifference(g[0][1], 1.9821442243860723) > EPS)
                update += 1;
              if (GetDifference(g[1][0], 1.9821442243860723) > EPS)
                update += 1;
              if (std::abs(g[0][2]) > EPS)
                update += 1;
              if (std::abs(g[2][0]) > EPS)
                update += 1;
              if (GetDifference(g[1][1], 15.78288823) > EPS)
                update += 1;
              if (GetDifference(g[1][2], -0.17903916) > EPS)
                update += 1;
              if (GetDifference(g[2][1], -0.17903916) > EPS)
                update += 1;
              if (GetDifference(g[1][3], -3.51690001) > EPS)
                update += 1;
              if (GetDifference(g[3][1], -3.51690001) > EPS)
                update += 1;
              if (GetDifference(g[2][2], 51.27859056) > EPS)
                update += 1;
              if (std::abs(g[2][3]) > EPS || std::abs(g[3][2]) > EPS)
                update += 1;
              if (GetDifference(g[3][3], 9.18405881) > EPS)
                update += 1;
              // Inverse metric
              Real ginv[NDFULL][NDFULL];
              system.SpacetimeMetricInverse(X[0], X[1], X[2], X[3], ginv);
              if (GetDifference(ginv[0][0], -1.63870625e+00) > EPS)
                update += 1;
              if (GetDifference(ginv[0][1], 2.05810289e-01) > EPS)
                update += 1;
              if (GetDifference(ginv[1][0], 2.05810289e-01) > EPS)
                update += 1;
              if (std::abs(ginv[0][3]) > EPS || std::abs(ginv[3][0]) > EPS)
                update += 1;
              if (GetDifference(ginv[1][1], 4.34252153e-02) > EPS)
                update += 1;
              if (GetDifference(ginv[1][2], 0.00015161910494636605) > EPS) {
                printf("Bad ginv[1][2] %g %g\n",ginv[1][2], 0.00015161910494636605);
                update += 1;
              }
              if (GetDifference(ginv[2][1], 0.00015161910494636605) > EPS)
                update += 1;
              if (GetDifference(ginv[1][3], 2.65272964e-02) > EPS)
                update += 1;
              if (GetDifference(ginv[3][1], 2.65272964e-02) > EPS)
                update += 1;
              if (GetDifference(ginv[2][2], 1.95018454e-02) > EPS)
                update += 1;
              if (GetDifference(ginv[3][3], 1.19042557e-01) > EPS)
                update += 1;
              ...
              constexpr Real Gamma_nubhlight[NDFULL][NDFULL][NDFULL] = {
                  {{6.45525676e-02, 5.14097064e-01, -3.30779101e-02,
                    -4.46414555e-02},
                   {5.14097064e-01, 2.56904200e+00, -0.06613399,
                    -3.55524840e-01},
                   {-3.30779101e-02, -0.06613399, -1.04594396e+01,
                    2.28750940e-02},
                   {-4.46414555e-02, -3.55524840e-01, 2.28750940e-02,
                    -1.68257280e+00}},
                  {{1.36203547e-02, -5.87986418e-02, -2.03004280e-13,
                    -9.41918320e-03},
                   {-5.87986418e-02, 5.04209748e-01, -4.40835235e-02,
                    2.61511601e-01},
                   {-2.03004280e-13, -4.40835235e-02, -2.20690336e+00,
                    -1.18727250e-12},
                   {-9.41918320e-03, 2.61511601e-01, -1.18727250e-12,
                    -3.55016683e-01}},
                  {{-5.97507320e-04, -2.20716631e-03, -7.08797170e-16,
                    8.24885520e-03},
                   {-2.20716631e-03, -9.62764242e-03, 1.13318725e+00,
                    6.46865963e-02},
                   {-7.08797170e-16, 1.13318725e+00, -3.33103600e+00,
                    -4.14512175e-15},
                   {8.24885520e-03, 6.46865963e-02, -4.14512175e-15,
                    -1.69890236e-01}},
                  {{8.32030847e-03, 2.59880304e-02, -4.78314158e-02,
                    -5.75392575e-03},
                   {2.59880304e-02, 8.27752972e-02, -3.76137106e-01,
                    9.69923918e-01},
                   {-4.78314158e-02, -3.76137106e-01, -1.34813792e+00,
                    9.42750263e-01},
                   {-5.75392575e-03, 9.69923918e-01, 9.42750263e-01,
                    -2.16870146e-01}}};
              SPACETIMELOOP3(sigma, mu, nu) {
                if (std::abs(Gamma_nubhlight[sigma][mu][nu]) > 0.1) {
                  if (GetDifference(Gamma[sigma][mu][nu],
                    ...
                    printf("%d %d %d: %g %g\n", sigma, mu, nu,
                           Gamma[sigma][mu][nu], Gamma_nubhlight[sigma][mu][nu]);
                    update += 1;
                  }
                } else {
